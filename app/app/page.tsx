...
import { FileSpreadsheet, LogOut, User, Loader2, Upload, History } from "lucide-react"
import { supabase } from "@/lib/supabase"

interface CleaningSession {
  id: string
  status: "uploading" | "analyzing" | "cleaning" | "completed"
  filename: string
  currentStep: number
  totalSteps: number
...
export default function AppPage() {
  ...
  const handleRecommendationDecision = async (recommendationId: string, decision: "accept" | "skip") => {
    ...
    console.log(`Making decision: ${decision} for recommendation: ${recommendationId}`)
    ...
    try {
      ...
      console.log("Apply recommendation response status:", response.status)
      ...
      if (!response.ok) {
        ...
        toast({
          title: "Error",
          description: errorData.error || "Failed to process recommendation",
          variant: "destructive",
        })
      ...
      const data = await response.json()
      console.log("Apply recommendation success:", data)
  ...
  const handleSelectFile = (sessionId: string) => {
    // This would load an existing session - for now just switch to upload tab
    setActiveTab("upload")
    toast({
              ...
              <div className="max-w-2xl mx-auto">
                <div className="text-center mb-8">
                  ...
                  <p className="text-gray-600 dark:text-gray-300">
                    Upload an Excel file (.xlsx) and let our AI assistant help you clean and prepare your data.
                  </p>
                </div>
